...
function ErrorModal({ message, clear }: { message: string; clear: Dispatch<SetStateAction<string>> }) {
  return (
    ...
    >
      <div className="relative w-[90%] sm:w-1/2 lg:w-1/3 xl:w-1/4 py-3 px-6 rounded-md bg-gray-100 dark:bg-gray-800 text-sm font-primary font-semibold text-red-500">
        <p className="text-center">Error: {message}</p>
        <button
          onClick={() => clear('')}
          className="absolute -right-2 -top-2 py-1 px-2 rounded-full bg-gray-50 dark:bg-gray-800 text-xs text-gray-800 dark:text-gray-200"
        >
          X
        </button>
      </div>
